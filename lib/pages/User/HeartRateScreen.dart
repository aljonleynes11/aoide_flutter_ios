...
class HeartRateScreen extends StatefulWidget {
  final String url;
  HeartRateScreen({Key key, @required this.url}) : super(key: key);
  @override
...
var slideText = [
  "The ideal resting heart rate for adults is 60 to 100 bpm.",
  "Target heart rates can be used to maximize the efficiency of your workouts, as well as to keep you safe.",
  "Typically, exercising at 60 to 85 percent of your maximum heart rate is most beneficial.",
];

LineChart lineChart = LineChart.fromDateTimeMaps([up], [Colors.red], ['BPM'],
...
class HeartRateSeries {
  ...
  HeartRateSeries(this.date, this.value, this.id);
}

List<HeartRateSeries> data = [];
var series = [
  new charts.Series<HeartRateSeries, DateTime>(
    id: 'HeartRate',
    colorFn: (_, __) => charts.MaterialPalette.red.shadeDefault,
    domainFn: (HeartRateSeries data, _) => data.date,
    measureFn: (HeartRateSeries data, _) => data.value,
    data: data,
  )
];
...
var chart = new charts.TimeSeriesChart(series,
    ...
    behaviors: [
      new charts.ChartTitle('Heart Rate',
...
class _HeartRateScreenState extends State<HeartRateScreen> {
                ...
                screenBody: Container(
                  decoration: BoxDecoration(
                    image: DecorationImage(
                        image: AssetImage(
                            "assets/images/backgrounds/background-red-cutted.png"),
                        fit: BoxFit.cover),
                  ),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await new Future.delayed(const Duration(milliseconds: 3), () {
          ...
          //         double.parse(x['heart_rate'].toString()));
          // print(chartFinal);
          // print(tempList['id'].toString());

          tempList.forEach((var x) => data.add(new HeartRateSeries(
  ...
  getDetails() async {
    ...
    try {
      final response =
          await get('https://www.aoide.tk/api/users/resting-heart-rate/$id');
  ...
  Future saveData() async {
    ...
    try {
      final response = await post(
        'https://www.aoide.tk/api/users/resting-heart-rate/$id',
      ...
      if (response.statusCode == 200) {
        var heartRate = heartRateController.text;
        await createLog('added', 'Added $heartRate heart rate');
        setState(() {
  ...
  deleteData(String deleteId) async {
    try {
      final response = await delete(
          'https://aoide.tk/api/users/resting-heart-rate/$deleteId',
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
          });
      final res = json.decode(response.body);
      if (response.statusCode == 200) {
        ...
        await createLog('deleted', 'Deleted heart rate');
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => HeartRateScreen(
                url:
                    'https://www.aoide.tk/api/users/paginate/resting-heart-rate/$getId?page=$currentPage',
              ),
            ));
